class Invidious::Routes::Feeds < Invidious::Routes::BaseRoute
  def playlists(env)
  ...
  def push_notifications_get(env)
  ...
  def push_notifications_post(env)
    locale = LOCALES[env.get("preferences").as(Preferences).locale]?
