...
def fetch_channel(ucid, pull_all_videos : Bool)
  ...
  channel_info = get_about_info(ucid, nil)

  channel = InvidiousChannel.new({
    id:         channel_info.ucid,
  ...
  LOGGER.trace("fetch_channel: #{ucid} : Extracting videos from channel response")
  videos.select(SearchVideo).each do |channel_video|
    ...
    LOGGER.trace("fetch_channel: #{ucid} : video #{channel_video.id} : Updating or inserting video")
    ...
    if was_insert
      ...
      LOGGER.trace("fetch_channel: #{ucid} : video #{channel_video.id} : Updated")
    end
  end
